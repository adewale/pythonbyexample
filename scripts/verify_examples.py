...
def main() -> int:
    ...
    paths = [EXAMPLES_DIR / "manifest.toml", *sorted(EXAMPLES_DIR.glob("*.md"))]
    for path in paths:
    ...
    for example in examples:
        ...
        errors.extend(f"{path.parent / error}" if error.startswith(f"{slug}.md:") else error for error in verify_example_output(example))
