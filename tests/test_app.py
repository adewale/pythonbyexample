...
class AppTests(unittest.TestCase):
    ...
    def test_example_page_contains_code_docs_and_run_form(self):
        ...
        self.assertIn('textarea { box-sizing: border-box; width: 100%; height: auto;', html)
    ...
    def test_ui_polish_principles_are_applied(self):
        html = render_example_page(get_example("hello-world"), output="hello world\n")
        self.assertIn("-webkit-font-smoothing: antialiased", html)
        self.assertIn("text-wrap: balance", html)
        self.assertIn("font-variant-numeric: tabular-nums", html)
        self.assertIn("transform: scale(0.96)", html)
        self.assertIn("nav a { color: inherit; text-decoration: underline", html)
        self.assertNotIn("nav a { min-height: 40px; display: inline-flex; align-items: center; border-radius", html)
        self.assertNotIn("transition: all", html)
        self.assertIn("min-height: 40px", html)
        self.assertIn("box-shadow:", html)
        self.assertIn("background: transparent", html)
        self.assertIn("border-left: 2px solid var(--accent)", html)
        self.assertIn("--space-6", html)
        self.assertIn("runner-grid", html)
    ...
    def test_cf_workers_design_system_and_playground_lessons(self):
        html = render_example_page(get_example("hello-world"), output="hello world\n")
        self.assertIn("#FF4801", html)
        self.assertIn("#F5F1EB", html)
        self.assertIn("#521000", html)
        self.assertIn("#EBD5C1", html)
        ...
        self.assertIn('class="output-panel"', html)
        self.assertIn("text-underline-offset", html)
        self.assertIn('aria-live="polite"', html)
        self.assertIn('min-height: 18rem', html)
        self.assertIn('white-space: pre-wrap', html)
        self.assertIn('overflow-wrap: anywhere', html)
        self.assertNotIn('max-height: 18rem', html)
